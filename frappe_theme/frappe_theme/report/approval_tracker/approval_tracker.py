...
def get_columns() -> list[dict]:
	return [
		...
		{"label": _("Rejected Requests"), "fieldname": "rejected_requests", "fieldtype": "Int", "width": 200},
	]
...
def get_data() -> list[list]:
	wf_list = frappe.get_all(
		"Workflow",
		filters={"is_active": 1},
		fields=["document_type", "workflow_state_field", "workflow_name"],
	)
	data = []
	for wf in wf_list:
		...
		approved_count = (
			frappe.db.count(wf.document_type, {wf.workflow_state_field: positive_closure})
			if positive_closure
			else 0
		)
		rejected_count = (
			frappe.db.count(wf.document_type, {wf.workflow_state_field: negative_closure})
			if negative_closure
			else 0
		)
