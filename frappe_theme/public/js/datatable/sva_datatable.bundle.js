...
class SvaDataTable {
	...
	createActionColumn(row, primaryKey) {
		...
		if (this.workflow) {
			appendDropdownOption(`${frappe.utils.icon("workflow", "sm")} ${__("Approval Actions")}`, async () => {
				open_approval_timeline_dialog(this.doctype, primaryKey, this.title_field ? row[this.title_field] : '');
			});
		}
		// ========================= Print Button ======================
