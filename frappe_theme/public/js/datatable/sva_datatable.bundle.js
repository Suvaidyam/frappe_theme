...
class SvaDataTable {
	...
	createActionColumn(row, primaryKey) {
		...
		const _dtTableKey = `${this?.doctype || this?.link_report || "unknown"}-${this?.connection?.html_field || "unknown"
			}`;
		...
		if (
			...
			const rowTitle = this.title_field && row[this.title_field] ? row[this.title_field] : row.name;
			...
			commentBtn.addEventListener("click", function (event) {
				...
				if (typeof window.openCommentsForDoc === "function") {
					let frm = Object.assign(self?.frm?.parent_frm || self?.frm, { child_row: Object.assign(row, { "doctype": rowDocType, "__title": rowTitle }) })
