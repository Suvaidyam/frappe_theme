...
class SvaDataTable {
	...
	async setupWrapper(wrapper) {
		wrapper.style = `max-width:${this.options?.style?.width || "100%"}; width:${this.options?.style?.width || "100%"
			};margin:0px !important; ${this.connection?.enable_card_view
	...
	createActionColumn(row, primaryKey) {
		...
		if (
			...
			countBadge.style.cssText = "position:absolute;top:-6px;right:-8px;background:#e0e0e0;color:#666;border-radius:50%;font-size:9px;min-width:14px;height:14px;display:none;align-items:center;justify-content:center;padding:0 2px;font-weight:bold;";
				...
				if (!window.__dtCommentSidebarObserver) {
					...
					if (sidebar) {
						window.__dtCommentSidebarObserver = new MutationObserver(function () {
							if (sidebar.style.display === "none" && typeof window.__dtActiveCommentRefresh === "function") {
								window.__dtActiveCommentRefresh();
								window.__dtActiveCommentRefresh = null;
							}
						});
						window.__dtCommentSidebarObserver.observe(sidebar, { attributes: true, attributeFilter: ["style"] });
					}
				}
			});
		}
		...
		if (this.rows.length === 0) {
			...
			this.columns.forEach((column, index) => {
				...
				if (index === 0) {
					...
					valueCell.style.cssText = `padding:0px;width:100%;vertical-align:middle;text-align:center;height:${this.columns.length * 32 + 30
						}px;line-height:32px;`;
				}
			});
