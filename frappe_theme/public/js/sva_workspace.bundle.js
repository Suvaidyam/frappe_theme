...
if (frappe?.workspace_block.blocks?.custom_block) {
	frappe.workspace_block.blocks.custom_block = class SVACustomBlock extends (
		frappe.workspace_block.blocks.custom_block
	) {
		constructor(...args) {
			super(...args);
			if (this.readOnly) {
		...
		async render_sva_custom_block() {
			...
			if (page_type == "Workspaces") {
				...
				let blocks = [
					...workspace_conf?.heatmaps.map((heatmap) => {
						return { ...heatmap, type: "Heatmap" };
					}),
					...workspace_conf?.tables.map((table) => {
						return { ...table, type: "SVATable" };
					}),
				];
				if (workspace_conf) {
					let block = blocks.find(
						(block) =>
							(block?.custom_block || block?.custom_html_block) ==
							this.data.custom_block_name
					);
					let wrapper = await get_wrapper(
						block?.custom_block || block?.custom_html_block
					);
