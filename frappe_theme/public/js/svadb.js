class SVAHTTP {
	...
	async fetchAPI(method, data) {
		...
		if (!response.ok) {
			...
			if (messages.length) {
				msg = messages
					.map((message_obj) => JSON.parse(message_obj || "{}")?.message)
					.join("\n");
			}
	...
	async exists(doctype, filters) {
		const exists = await this.fetchAPI("frappe.client.get_value", {
			doctype,
			fieldname: "name",
			filters,
		});
	...
	async set_value(doctype, name, fieldname, value) {
		let res = await this.fetchAPI("frappe.client.set_value", {
			doctype,
			name,
			fieldname,
			value,
