			...
			fields: [
				{
					...
					options: frm.doc.fields
						.filter(
							(e) =>
								!["Section Break", "Column Break", "Tab Break", "HTML"].includes(
									e.fieldtype
								)
						)
						.map((f) => {
...
function add_customization_buttons(frm) {
	frm.add_custom_button(
		...
		function () {
			frappe.confirm("Are you sure you want to export customizations?", () => {
				if (!frm.doc.doc_type) {
					frappe.msgprint(
						__("Please select a DocType first before exporting customizations.")
					);
					return;
				}
				frappe.prompt(
	...
	// ---- IMPORT BUTTON ----
	frm.add_custom_button(
		__("Import Customizations"),
		function () {
			if (!frm.doc.doc_type) {
				frappe.msgprint(
					__("Please select a DocType first before importing customizations.")
				);
				return;
			}
					...
					frappe.call({
						...
						callback: function (r) {
							if (!r.exc) {
								...
								frappe.msgprint(
									__("Failed to import customizations. Check error logs.")
								);
							}
						},
					});
