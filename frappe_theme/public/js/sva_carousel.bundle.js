...
class SVACarousel {
	constructor({ wrapper, conf, html_field, frm }) {
		...
		this.touchStartX = 0;
		this.touchEndX = 0;
		this.forceCardLayout = this.getColumnsFromUI(html_field, frm) >= 3;
	...
	/* ================= Helper Methods ================= */
	getColumnsFromUI(fieldname, frm) {
	...
	applyConfigOverrides(config, deviceType) {
		...
		if (this.conf?.carousel_heightin_px) {
			config.carouselHeight = this.conf.carousel_heightin_px;
			if (isMobile) {
				config.imgHeight = `${Math.floor(this.conf.carousel_heightin_px * 0.6)}px`;
			}
		}
		if (this.conf?.title_font_size) {
			config.titleFontSize = Math.max(
				this.conf.title_font_size * multipliers[deviceType],
				isMobile ? 14 : 18
			);
		}
		if (this.conf?.desc_font_size) {
			config.descFontSize = Math.max(
				this.conf.desc_font_size * (multipliers[deviceType] + 0.05),
				isMobile ? 11 : 14
			);
		}
		return config;
	...
	getIndicatorPositionStyles(indicatorPosition, config, isMobile, imgHeight = null) {
	...
	generateMediaHTML(cd) {
		const mediaType = this.getMediaType(cd.img);
		if (mediaType === "video") {
			const isEmbed =
				cd.img.includes("youtube") ||
				cd.img.includes("vimeo") ||
				cd.img.includes("youtu.be");
			if (isEmbed) {
				const embedUrl = this.getEmbedUrl(cd.img);
				...
				${this.data
					.map(
						(
							cd,
							i
						) => `<div class="container" data-index="${i}" data-media-type="${this.getMediaType(
							cd.img
						)}">
					${this.generateMediaHTML(cd)}
	...
	playCurrentVideo(container) {
		...
		if (fgVideo && this.conf?.auto_play !== false) {
				...
				if (r.message && r.message.result) {
					...
					if (!hasMediaField) {
						this.showErrorMessage(
							...
							`The selected report must contain at least one of these fields:<br><strong>img</strong>, <strong>image</strong>, or <strong>video</strong>`,
					...
					this.data = r.message.result
						.filter((row) => row.img || row.image || row.video)
						.map((row) => ({
		...
		this.wrapper.html(`
			...
			">
				<div>
					...
					${details ? `<p style="color: #868e96; margin: 16px 0 0 0; font-size: 12px;">${details}</p>` : ""}
				</div>
			</div>
		`);
	}
	...
	init() {
		...
		this.wrapper.html(this.generateCarouselHTML());
	...
	applyStyles() {
		...
		let config = { ...this.responsiveConfig[deviceType] };
		config = this.applyConfigOverrides(config, deviceType);
		...
		// Container styles
		this.containers.each((idx, container) => {
			...
			if (isMobile) {
				...
				if (c) {
					...
					Object.assign(c.style, {
						textAlign: "center",
						fontFamily: this.conf?.font_family || "system-ui",
						color: "#333",
				...
				if (t) {
					Object.assign(t.style, {
						color: this.conf?.title_color || "#1a1a1a",
						background: "transparent",
						textAlign: this.conf?.alignment || "center",
						fontFamily: this.conf?.font_family || "system-ui",
				...
				if (d) {
					Object.assign(d.style, {
						color: this.conf?.desc_color || "#4a4a4a",
						background: "transparent",
						textAlign: this.conf?.alignment || "center",
						fontFamily: this.conf?.font_family || "system-ui",
				...
				if (c) {
					Object.assign(c.style, {
						textAlign: this.conf?.alignment || "left",
						fontFamily: this.conf?.font_family || "system-ui",
						color: this.conf?.text_color || "#fff",
						width: "100%",
						maxHeight: "100%",
						overflowY: "auto",
						overflowX: "hidden",
				...
				if (t) {
					Object.assign(t.style, {
						color: this.conf?.title_color || "#fff",
						background: "transparent",
						textAlign: this.conf?.alignment || "left",
						fontFamily: this.conf?.font_family || "system-ui",
						fontSize: `${config.titleFontSize}px`,
				...
				if (d) {
					Object.assign(d.style, {
						color: this.conf?.desc_color || "#fff",
						background: "transparent",
						textAlign: this.conf?.alignment || "left",
						fontFamily: this.conf?.font_family || "system-ui",
						fontSize: `${config.descFontSize}px`,
		...
		this.wrapper.find(".nav").each((i, nav) => {
			Object.assign(nav.style, {
				border: "none",
				position: "absolute",
				top: navTop,
				transform: "translateY(-50%)",
				background: this.conf?.slider_button_bg || "rgba(255,255,255,0.9)",
			...
			nav.addEventListener("mouseenter", () =>
				Object.assign(nav.style, {
					background: this.conf?.slider_button_bg || "rgba(255,255,255,1)",
		...
		this.wrapper.find(".prev")[0].style.left = config.navButtonPosition;
		this.wrapper.find(".next")[0].style.right = config.navButtonPosition;

		this.wrapper.find(".nav svg").each((i, svg) => {
			Object.assign(svg.style, {
				color: this.conf?.slider_button_color || "#111",
		...
		// Indicators
		const indicatorType = this.conf?.bullet_indicator_type || "Bullets";
		const indicatorPosition = this.conf?.indicator_position || "Bottom Center";
		...
		this.wrapper.find(".bullet").each((idx, bullet) => {
		...
		const pageCounter = this.wrapper.find(".page-counter")[0];
		if (pageCounter) {
			const showCounter =
	...
	updateSlide() {
		...
		if (this.pageCounterCurrent) {
			this.pageCounterCurrent.textContent = this.currentIndex + 1;
		}
	}
	...
	destroy() {
		this.stopAuto();
		this.containers.each((i, c) => this.pauseAllVideos(c));
