...
async function getDoctypeWithChildFields(doctype) {
  // Fetch parent meta
  const metaRes = await frappe.call({
    method: "frappe.desk.form.load.getdoctype",
    args: { doctype }
  });

  if (!metaRes.docs || metaRes.docs.length === 0) {
    throw new Error(`Meta not found for doctype: ${doctype}`);
  }

  const meta = metaRes.docs[0];

  // Parent fields (exclude children, table fields, read-only, etc.)
  const parentFields = meta.fields
    .filter(f => f.fieldtype !== "Table" && !f.read_only)
    .map(f => f.fieldname);

  // Child table info
  const childTables = meta.fields
    .filter(f => f.fieldtype === "Table")
    .map(f => ({ fieldname: f.fieldname, childDoctype: f.options }));

  return { parentFields, childTables };
}
async function getRecordsWithChildren(doctype) {
  const { parentFields, childTables } = await getDoctypeWithChildFields(doctype);

  // Fetch parent records
  const parentRes = await frappe.call({
    method: "frappe.client.get_list",
    args: {
      doctype,
      fields: ["name", ...parentFields],
      limit_page_length: 100
    }
  });

  if (!parentRes.message) return [];

  const parentRecords = parentRes.message;

  // Fetch child table data for each parent
  for (let record of parentRecords) {
    for (let child of childTables) {
      const childData = await frappe.call({
        method: "frappe.client.get_list",
        args: {
          doctype: child.childDoctype,
          filters: { parent: record.name, parentfield: child.fieldname, parenttype: doctype },
          fields: ["*"]  // fetch all child fields dynamically
        }
      });
      record[child.fieldname] = childData.message || [];
    }
  }
...
onMounted(async () => {
  ...
  try {
    ...
    const testDoc = await frappe.call({
      ...
      args: {
        doctype: "Excel JSON Wb",
        fields: ["name","first_name", "last_name", "gender", "age","dob","country","state"],
...
// ================ insertwithexcelchildtable ================
// async function insertOrUpdateExcelRecord(row) {
//   try {
//     // ====== Parent Meta ======
//     const metaRes = await frappe.call({
//       method: "frappe.desk.form.load.getdoctype",
//       args: { doctype: "Excel JSON Wb" }
//     });
//     const meta = metaRes.docs[0];

//     const parentFields = meta.fields.map(f => f.fieldname);

//     // 🔹 Find first child table field dynamically
//     const childTableField = meta.fields.find(f => f.fieldtype === "Table");
//     if (!childTableField) {
//       throw new Error("No child table field found in parent meta!");
//     }

//     const childDoctype = childTableField.options;   // actual child doctype
//     const childFieldname = childTableField.fieldname; // parentfield

//     // ====== Child Meta ======
//     const childMetaRes = await frappe.call({
//       method: "frappe.desk.form.load.getdoctype",
//       args: { doctype: childDoctype }
//     });
//     const childMeta = childMetaRes.docs[0];
//     const childFields = childMeta.fields.map(f => f.fieldname);

//     // ====== Parent fields extract from row ======
//     let parentData = {};
//     for (let key of Object.keys(row)) {
//       if (parentFields.includes(key)) {
//         parentData[key] = row[key];
//       }
//     }

//     // ====== Child row extract from row ======
//     let childRow = {};
//     for (let key of Object.keys(row)) {
//       if (childFields.includes(key)) {
//         childRow[key] = row[key];
//       }
//     }

//     let childData = [];
//     if (Object.keys(childRow).length > 0) {
//       childData.push({
//         doctype: childDoctype,
//         parentfield: childFieldname,
//         parenttype: "Excel JSON Wb",
//         ...childRow
//       });
//     }
...
//         updatedDoc[childFieldname] = childData;
//       }
...
//       });
//       updatedCount++;
//       console.log("Updated with child:", rowId);
//       return res.message;
...
//       if (childData.length > 0) {
//         newDoc[childFieldname] = childData;
//       }
...
//         args: { doc: newDoc }
//       });
//       console.log("Inserted with child:", res.message.name);
//       createdCount++;
//       return res.message;
//     }
//   } catch (err) {
