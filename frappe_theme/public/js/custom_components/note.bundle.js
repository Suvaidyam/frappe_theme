class SVANotesManager {
	...
	async fetchNotes() {
		const response = await frappe.call({
			...
			args: {
				doctype: "Notes",
				fields: ["name", "title", "description", "creation", "owner"],
				filters: {
	...
	async handleEditNote(noteName) {
		try {
			...
			if (!meta || !meta.fields) {
				frappe.show_alert({
					message: "Failed to fetch doctype metadata",
					indicator: "red",
				});
				return;
			}
			...
			while (formContainer.firstChild) {
				// Clear existing form using while loop
			...
			updateButton.addEventListener("click", async () => {
				...
				fieldControls.forEach((control) => {
					...
					if (control.df.reqd && !value) {
						...
						frappe.msgprint({
							message: `Field "${control.df.label}" is mandatory`,
							indicator: "red",
						});
					}
	...
	async handleDeleteNote(noteName) {
		frappe.confirm("Are you sure you want to delete this Note?", async () => {
			try {
			...
			submitButton.addEventListener("click", async () => {
				...
				try {
					...
					if (message) {
						...
						fieldControls.forEach((control) => {
							if (
								!["reference_doctype", "related_to"].includes(control.df.fieldname)
							) {
								control.set_value("");
							}
						});
