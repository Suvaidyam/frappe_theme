class SVALinkedUser {
						...
						this.user_list.length === 0
                            ...
                            <tr class="grid-row">
                                <td class="row-check sortable-handle col" style="width: 40px; text-align: center; position: sticky; left: 0px; background-color: #fff;">
                                   <!-- <input type="checkbox" class="toggleCheckbox" data-id="${
										user.name
									}"> -->
                                ...
                                <td style="padding: 5px 8px !important;">
                                    ...
                                    ${
                                        ...
                                        <div class="dropdown-menu" aria-labelledby="dropStatus-${
                                            ...
                                            <a class="dropdown-item user-status" data-user="${
												user.name
							...
							frappe.confirm(
								...
								async () => {
									this.user_list = this.user_list.filter(
										(user) => !selectedIds.includes(user.name)
									);
				...
				.on(
					...
					function (e) {
						...
						$("#selectAllCheckBox").prop(
							"checked",
							selectedIds.length === this.user_list.length
						);
	...
	async form(data = null, action, frm) {
		...
		let fields = fileds?.docs[0]?.fields
			.filter((field) =>
				["role_profile", "first_name", "last_name", "email", "mobile_number"].includes(
					field.fieldname
				)
			)
			.map((field) => {
				...
				if (action === "New User") {
					...
					frappe.db
						...
						.catch((error) => {
							console.error(error);
							frappe.show_alert({
								message: __("There was an error creating the user"),
								indicator: "red",
							});
						});
					...
					frappe.db
						.set_value("SVA User", data.name, values)
						.then((updated_doc) => {
							if (updated_doc) {
								frappe.show_alert({
						...
						.catch((error) => {
							console.error(error);
							frappe.show_alert({
								message: __("There was an error updating the user"),
								indicator: "red",
							});
						});
				}
