...
class SVAHeatmap {
	...
	render() {
		...
		try {
			...
			$(this.wrapper).html(
				'<div class="alert alert-danger">Failed to initialize map.</div>'
			);
		}
	}
			...
			callback: (r) => {
				if (r.message) {
					...
					if (!this.stateField && hasStateColumn) {
						this.stateField = this.reportData.columns.find(
							(col) => col.options === "State"
						).fieldname;
					}
					const hasDistrictColumn = this.reportData.columns.some(
						(col) => col.options === "District"
					);
					if (!this.districtField && hasDistrictColumn) {
						this.districtField = this.reportData.columns.find(
							(col) => col.options === "District"
						).fieldname;
					}
					if (this.stateField || hasStateColumn) {
						...
						this.hideLoader();
						$(this.wrapper).html(
							'<div class="alert alert-warning">Report does not have a State column.</div>'
						);
					}
	...
	createLegend(range) {
		...
		breaks.forEach((break_, index) => {
			...
			if (nextBreak) {
				...
				$("<div>").text(rangeText).appendTo(legendItem);
