        ...
        if (field_meta?._link_field) {
            if (Array.isArray(values)) {
                ...
                for (const val of values) {
                    ...
                    const displayVal = await this.fetchLinkFieldValue(field_meta?._link_field?.options, value);
            ...
            if (first_link_field) {
                if (Array.isArray(values)) {
                    ...
                    for (const val of values) {
                        ...
                        const displayVal = await this.fetchLinkFieldValue(first_link_field?.options, value);
                        displayValues.push(displayVal);
                    }
                    return displayValues.join(', ');
                }
        ...
        if (Array.isArray(filters)) {
            for (const filter of filters) {
                ...
                if (Array.isArray(filter) && filter.length >= 3) {
                    ...
                    const formatted = await this.formatFilter(fieldname || doctype, operator, value);
                    parsed.push(formatted);
                }
        ...
        this.ribbonElement.innerHTML = `
            <div class="filter-ribbon-content">
                ...
                <div class="filter-text">
                    <strong>${filterCount} filter${filterCount > 1 ? 's' : ''} applied:</strong> ${filterText}
