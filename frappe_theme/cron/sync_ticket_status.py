...
def run():
	...
	sva_tickets = frappe.db.get_all(
		"SVA Ticket", filters={"status": ["!=", "Closed"]}, fields=["name", "hd_id"]
	)
	...
	hd_tickets = get_hd_tickets(mainBaseURL, mainCreds, {"name": ["IN", hd_names]})

	for hd in hd_tickets:
		sva_ticket = frappe.db.get_value(
			"SVA Ticket", {"hd_id": hd["name"]}, ["name", "status", "priority", "description"], as_dict=True
		)
