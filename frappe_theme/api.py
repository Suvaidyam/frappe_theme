...
@frappe.whitelist()
def get_user_wise_transitions(
    doc: Union["Document", str, dict], user = None
) -> list[dict]:
...
def get_workflow_based_users(doctype):
    ...
    role_users = frappe.get_all(
        "Has Role",
        filters={"role": ["in", list(transition_roles)], "parenttype": "User", "parent": ["!=", frappe.session.user]},
        pluck="parent",
    )
    
    users = frappe.get_all("User", filters={"name": ["in", role_users]}, fields=["email as value", "full_name as label"])
