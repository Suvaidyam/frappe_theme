			...
			if doctype:
				...
				for idx, fieldname in enumerate(result["keys"]):
					...
					if not df:
						continue
					cfg = get_data_protection(df)
					if cfg.get("encrypt") or (
						cfg.get("masking") and "list" in cfg["masking"].get("apply_masking_on", [])
					):
...
def decrypt_value(value):
	...
	try:
		...
		logger.warning("Invalid token during decryption, returning original value")
		return value
	except Exception as e:
