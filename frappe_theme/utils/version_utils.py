	...
	def _apply_field_level_permissions(results, parent_doctype):
		...
		def get_cached_permitted_fields(doctype, parenttype=None):
			...
			if cache_key not in permitted_fields_cache:
				...
				# Table / Table MultiSelect fields are not stored as columns on the
				# parent table, so get_permitted_fields() excludes them. Add them
				# back so audit log entries for Thematic Areas etc. are not dropped.
				try:
					meta = frappe.get_meta(doctype)
					for df in meta.fields:
						if df.fieldtype in ("Table", "Table MultiSelect") and (df.permlevel or 0) == 0:
							fields.add(df.fieldname)
				except Exception:
					pass
				permitted_fields_cache[cache_key] = fields
