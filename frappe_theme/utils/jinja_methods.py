...
def approver_details(dt, dn, workflow_state=""):
	try:
		...
		if frappe.db.exists(
			"Workflow Action", {"reference_doctype": dt, "reference_name": dn, "status": "Completed"}
		):
			if workflow_state:
				wa = frappe.get_list(
					"Workflow Action",
					...
					fields=["completed_by", "completed_by_role"],
					ignore_permissions=True,
				)
			else:
				wa = frappe.get_list(
					"Workflow Action",
					filters={"reference_doctype": dt, "reference_name": dn, "status": "Completed"},
					fields=["completed_by", "completed_by_role"],
					ignore_permissions=True,
				)
			if len(wa) > 0:
				user_details = frappe.get_list(
					"SVA User",
					filters={"email": wa[0].completed_by},
