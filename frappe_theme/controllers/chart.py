...
class Chart:
    ...
    def chart_report(
        ...
        try:
            ...
            if report.get("report_type") == "Query Report" and report.get("query"):
                ...
                if details.get("type") == "Bar":
                    chart_data = Chart.process_response_for_bar_chart(details, data, columns)
    ...
    @staticmethod
    def process_response_for_bar_chart(details, data, columns):
        y_axis_columns = details.get("y_axis", [])
        data_sets = []
        labels = []
        data_sets_map = {}
        ...
        for y_axis in y_axis_columns:
            ...
            data_sets_map[y_field] = {
                ...
                "borderColor": y_axis.get("color") if (y_axis.get("custom_type") or details.get("chart_type")).lower() == "line" else None,
                "type": (y_axis.get("custom_type") or details.get("chart_type")).lower(),
                "label": next(
        ...
        for row in data:
            ...
            if isinstance(row, dict):
                label = row.get(details.get("x_field")) or "Unknown"
            else:
                x_idx = next(
                    (i for i, col in enumerate(columns)
                    if col.get("fieldname") == details.get("x_field")),
                    None,
                )
            ...
            for y_field, dataset in data_sets_map.items():
                dataset["data"].append(row_values[y_field])
            ...
            "data": {
                ...
                "options": {
                    "scales": {
                        "x": {
                            "ticks": {
                                "maxRotation": int(details.get("custom_rotate_values").rstrip("°") if details.get("custom_rotate_values") else 0),
                                "minRotation": 0,
                                "autoSkip" : True
                            }
                        },
