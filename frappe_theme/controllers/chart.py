	...
	def process_response_for_bar_chart(details, data, columns):
		...
		for y_axis in y_axis_columns:
			...
			data_sets_map[y_field] = {
				...
				"borderColor": y_axis.get("color")
				if (y_axis.get("custom_type") or details.get("chart_type")).lower() == "line"
				else None,
		...
		for row in data:
			...
			if isinstance(row, dict):
				...
				x_idx = next(
					(i for i, col in enumerate(columns) if col.get("fieldname") == details.get("x_field")),
					None,
				)
				...
				"options": {
					"scales": {
						"x": {
							"ticks": {
								"maxRotation": int(
									details.get("custom_rotate_values").rstrip("°")
									if details.get("custom_rotate_values")
									else 0
								),
								"minRotation": 0,
								"autoSkip": True,
