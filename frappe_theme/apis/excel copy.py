...
#     wb = load_excel_from_private("/private/files/excel_dropdown_example (15).xlsx")
...
#     """
#     Fetch parent + child records dynamically, bypassing permission issues
#     """
#     def fetch_parent():
#         meta = frappe.get_meta("Excel JSON Wb")
#         parent_fields = [f.fieldname for f in meta.fields if f.fieldtype not in ["Section Break","Column Break","Table","Button"]]
#         parent_fields.insert(0, "name")  # always include name
#         return frappe.get_list("Excel JSON Wb", fields=parent_fields, order_by="creation asc", limit_page_length=100)

#     def fetch_children(parent_names):
#         meta = frappe.get_meta("Excel JSON Wb")
#         child_tables = [f for f in meta.fields if f.fieldtype == "Table"]
#         all_child_data = {}
#         for child in child_tables:
#             child_meta = frappe.get_meta(child.options)
#             child_fields = [f.fieldname for f in child_meta.fields if f.fieldtype not in ["Section Break","Column Break","Table","Button"]]
#             child_fields.insert(0, "name")
#             child_fields.insert(1, "parent")
#             docs = frappe.get_list(child.options, fields=child_fields, filters={"parent": ["in", parent_names]})
#             for d in docs:
#                 if d.parent not in all_child_data:
#                     all_child_data[d.parent] = {}
#                 if child.fieldname not in all_child_data[d.parent]:
#                     all_child_data[d.parent][child.fieldname] = []
#                 all_child_data[d.parent][child.fieldname].append(d)
#         return all_child_data

#     parent_records = fetch_parent()
#     parent_names = [p.name for p in parent_records]
#     child_records_map = fetch_children(parent_names)

#     return {
#         "parent": parent_records,
#         "child": child_records_map
#     }
